...
class NeuralSequenceDecoder(object):
    ...
    def __init__(self, args):
        ...
        self.model = transformermodels.PartialDecoder(
    ...
    def train(self):
        ...
        for batchIdx in range(restoredStep, self.args["nBatchesToTrain"] + 1):
            ...
            if batchIdx % self.args["batchesPerVal"] == 0 and batchIdx != 0:
                ...
                if self.max_wer < avg_wer and avg_wer < 10:
    ...
    def _trainStep(self, datasetIdx, layerIdx):
        ...
        with tf.GradientTape() as tape:
            ...
            predictions, _ = self.model(data["seqClassIDs"][:,:-1], inputTransformedFeatures, True, look_ahead_mask, dec_mask)
    ...
    def _valStep(self, data, layerIdx, infer=False):
        ...
        for i in range(step):
            ...
            # Only Decoder
            predictions, _ = self.model(outputLabels, inputTransformedFeatures, False, look_ahead_mask, dec_mask)
        ...
        print("Output : " + str(output_sent[0][:target_length[0]]))
        
        s_wer = 0
        for idx in range(len(target_sent)):
            # s_wer += wer(target_sent[idx], output_sent[idx])
            s_wer += calculate_cer(target_sent[idx][1:-2], output_sent[idx][:-1])
            # print(s_wer)
