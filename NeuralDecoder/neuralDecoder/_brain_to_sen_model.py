...
class DecoderLayer(tf.keras.layers.Layer):
  ...
  def call(self, x, enc_output,
    ...
    attn2, attn_weights_block2 = self.mha2(
        enc_output, enc_output, out1, None, training=training)  # (batch_size, target_seq_len, d_model)
...
class Encoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff,
    ...
    self.dense3 = tf.keras.layers.Dense(d_model, kernel_constraint=constraints.MaxNorm(max_value=20.0), activation='gelu')
...
class BrainToSenTransformer(tf.keras.Model):
  ...
  def call(self, inputs, training=False):
    ...
    for_padding = tf.argmax(inp, axis=-1)
    
    enc_padding_mask, look_ahead_mask, dec_padding_mask = self.create_masks(for_padding, tar)
    
    enc_output = self.encoder(inp, training=training)  # (batch_size, inp_seq_len, d_model)

    # dec_output.shape == (batch_size, tar_seq_len, d_model)
    dec_output, attention_weights = self.decoder(
        tar, enc_output, look_ahead_mask, training=training)
  ...
  def convert_chr_to_token(self, tar):
    
    s = self._ascii_to_text(tar)
    print(s)
    return
  
  def _ascii_to_text(self, text):
    s_list = []
    for i in range(len(text)):
        endIdx = np.argwhere(text[i] == 0)
        sen = "".join([chr(char) for char in text[i][0 : endIdx[0, 0]]])
        sen = "<s>" + sen
        s_list.append(sen)
